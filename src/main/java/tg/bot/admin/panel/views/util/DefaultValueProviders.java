...
public class DefaultValueProviders {
    ...
    public static String generateSellingItemName(SellingItem sellingItem) {
        if (ObjectUtils.isEmpty(sellingItem) || ObjectUtils.isEmpty(sellingItem.getCandy())) {
            return "N/A";
        }
        return String.format("%s - %sg.", sellingItem.getCandy().getName(), sellingItem.getWeight());
    }
